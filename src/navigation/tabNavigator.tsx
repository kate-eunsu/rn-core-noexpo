...
type TabNavigatorProp = StackNavigationProp<RootStackParamList, 'TabNavigator'>;
  ...
  return (
    <Tab.Navigator
      screenOptions={({route}) => ({
        tabBarIcon: ({color}) => {
          const tabIcon =
            tabIcons.find(icon => icon.key === route.name) ?? tabIcons[0];
          ...
          return (
            <View style={{alignItems: 'center', width: 56}}>
              <TabBarIcon
                ...
                label={tabIcon.name}
                color={color}
                strokeWidth={tabIcon.strokeWidth ?? 0}
              />
            </View>
          );
        },
      ...
      <Tab.Screen
        ...
        initialParams={{id: 2}}
      />
      <Tab.Screen name="Benefit" component={BenefitScreen} />
    </Tab.Navigator>
  );
};
